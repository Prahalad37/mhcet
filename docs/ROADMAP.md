...
- [x] **Phase 2 — Product polish** — Results: accuracy %, attempt summary, green/red review, inline static explanations. Take test: exam-style sticky palette, mark for review, keyboard navigation, non-blocking saves + flush-before-submit, memoized controls; **Submit test** only on last question. Shared visible loading copy via `PageLoadingState`.
